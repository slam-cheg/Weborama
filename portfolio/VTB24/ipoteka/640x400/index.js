    ...
    return fetch("./cities.json", {
        ...
        .then((res) => {
            for (let i = 0; i < res.length; i++) {
                ...
                if (Math.abs(latitude - res[i].latitude) < 2 && Math.abs(longitude - res[i].longitude) < 2) { // region check
                    if (Math.abs(latitude - res[i].latitude) < 0.5 && Math.abs(longitude - res[i].longitude) < 0.5) { // city check
